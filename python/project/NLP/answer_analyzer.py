...
def add_training_data():
    ...
    connection = pyodbc.connect(
        r'Driver={SQL Server};Server=cc414-nb-mldb.c6aodczcoukl.us-east-1.rds.amazonaws.com;Database=nlp;UID=nbsa;pipPWD=qwer1234;')
    cursor = connection.cursor()
    cursor.execute('SELECT name, content FROM concepts join definitions on concepts.id = definitions.conceptid')
    while 1:
        row = cursor.fetchone()
        if not row:
            break
